...
try:
    from ArisuFxPy import ArisuFxPyBoost
except ImportError:
    ArisuFxPyBoost = None
...
class ArchiveStorageDecryptor:
            ...
            raise LookupError(
                "\n".join(
                    [
                        ...
                        "You can set the key via ArisuFxPy.set_assetbundle_decrypt_key(key).",
                        "To try brute-forcing the key, use ArisuFxPy.helpers.ArchiveStorageManager.brute_force_key(fp, key_sig, data_sig)",  # noqa: E501
    ...
    def decrypt_block(self, data: bytes, index: int):
        if ArisuFxPyBoost:
            return ArisuFxPyBoost.decrypt_block(self.index, self.substitute, data, index)
