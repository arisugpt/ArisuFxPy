...
def get_fallback_version():
    ...
    if not isinstance(FALLBACK_UNITY_VERSION, str):
        raise UnityVersionFallbackError(
            ...
            + "Please explicitly set the value of ArisuFxPy.config.FALLBACK_UNITY_VERSION."
        )

    warnings.warn(
        f"No valid Unity version found, defaulting to ArisuFxPy.config.FALLBACK_UNITY_VERSION ({FALLBACK_UNITY_VERSION})",  # noqa: E501
